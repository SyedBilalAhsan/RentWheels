...
router.get("/",ensureAuthenticated, async (req, res) => {
  const user = await ReserveModel.findOne({ userid: req.user._id });
  if (user) {
  ...
  res.render("reserve", { records1: null });
});
router.post("/:id", async function (req, res) {
  ...
  try {
    var details = await ReserveModel.findOne({ userid: req.user._id });
    if (!details) {
    ...
    if (_id) {
      var car = await ReserveModel.findByIdAndUpdate(_id, { vid });
    }
  } catch (error) {
    console.log(error);
  }
