...
router.get("/", async (req, res) => {
  const user = await RegisterModel.findOne({ userid: req.user._id });
  if (user) {
  ...
  res.render("yourcar", { records1: null });
});
router.post("/:id", async function (req, res) {
  ...
  try {
    var details = await RegisterModel.findOne({ userid: req.user._id });
    if (!details) {
    ...
    if (_id) {
      var car = await RegisterModel.findByIdAndUpdate(_id, { vid });
    }
  } catch (error) {
    console.log(error);
  }
